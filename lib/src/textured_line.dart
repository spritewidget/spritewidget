...
class TexturedLinePainter {
  ...
  void paint(Canvas canvas) {
    ...
    if (texture != null) {
      ...
      double xPos = _xPosForStop(stops?[0]??(throw ArgumentError('Index 0 not found in stops or stops is null')));
    ...
    for (int i = 1; i < _points.length; i++) {
      ...
      if (texture != null) {
        // Texture coordinate points
        double xPos = _xPosForStop(stops?[i]??(throw ArgumentError('$i not found in stops or stops is null')));
        textureCoordinates.add(new Offset(xPos, textureTop));
        textureCoordinates.add(new Offset(xPos, textureBottom));
      }
  ...
  void _addVerticesForPoint(List<Offset> vertices, Offset point, Vector2 miter, double width) {
    ...
    if (removeArtifacts && vertexCount >= 2) {
      ...
      Offset intersection = GameMath.lineIntersection(oldVertex0, oldVertex1, vertex0, vertex1)??Offset.zero;
